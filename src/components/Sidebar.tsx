...
export default function Sidebar({
  ...
  return (
    <div className="w-full lg:w-[280px] bg-slate-950 border-b lg:border-b-0 lg:border-r border-slate-800 flex flex-col shrink-0">
      {/* Brand Header */}
      <div className="p-6 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-sans font-bold tracking-tight text-slate-100 text-lg">
              Depósito Fácil
            </h1>
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <div className="flex items-center gap-1">
                <span className={`w-1.5 h-1.5 rounded-full ${syncStatus === 'syncing' ? 'bg-amber-400 animate-pulse' : 'bg-red-500'}`} />
                <span className={`text-[9px] font-mono font-bold uppercase tracking-wider ${syncStatus === 'syncing' ? 'text-amber-400' : 'text-red-500'}`}>
        ...
        <button
          ...
          className="lg:hidden p-2 rounded-xl border border-slate-800 bg-slate-900 text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition-all focus:outline-none"
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} lg:block flex-1 flex flex-col min-h-0`}>
        ...
        {currentUser.role !== UserRole.SUPERADMIN && (
          <div className="p-4 mx-4 my-4 rounded-xl bg-slate-900 border border-slate-800 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <span className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-800 text-red-400 border border-slate-700">
                  {currentUser.role}
                </span>
              </div>
            </div>
              ...
              <div className="pt-2 border-t border-slate-800/60">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full text-xs bg-slate-950 border border-slate-800 rounded-lg py-1.5 px-2.5 text-slate-200 focus:outline-none focus:border-red-500 appearance-none"
                  >
              ...
              {allowedMenuItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-sans text-sm font-medium transition-all ${
                      activeTab === item.id
                        ? "bg-red-500/10 text-red-400 border-l-2 border-red-400 shadow-[inset_0_0_12px_rgba(239,68,68,0.04)]"
                        : "text-slate-400 hover:bg-slate-900 hover:text-slate-200"
                    }`}
                  >
        ...
        <div className="p-4 m-4 rounded-xl bg-slate-900 border border-slate-800/80 flex flex-col gap-2">
          ...
          <div className="space-y-1.5 mt-1">
            {/* Company switcher */}
            <div>
              <span className="text-[9px] font-mono text-slate-500">Locatário/Empresa</span>
              <select
                ...
                className="w-full text-[11px] bg-slate-950 border border-slate-800 rounded py-1 px-2 text-slate-300"
              >
            ...
            {/* User switcher */}
            <div>
              <span className="text-[9px] font-mono text-slate-500">Usuário & Cargo</span>
              <select
        ...
        <div className="p-4 border-t border-slate-800 flex items-center justify-between mt-auto">
          ...
          <button 
            onClick={onLogout}
            className="p-2 hover:bg-slate-900 rounded-lg text-slate-400 hover:text-red-400 transition-colors"
